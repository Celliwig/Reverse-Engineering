...
And this function is more interesting. Firstly it executes command '0xa0', loading in the destination address as an arguement. Having executed that command it sets command '0xb8' and loops through 8 bytes of source data, setting them as arguements to that command, before executing it. This last sequence is then repeated for the remaining 4kB of data. This matches the listed commands, but in this case the string 'CMD_PR=2$A0B8$ADR=1$03$' is a little misleading as it would suggest one command instead of two. One other thing to mention is that I was hoping that the 8 in '0xb8' was a byte count, and the number of bytes written could be controlled (to a degree). Sadly when I tried the command '0xb1', the MCU failed to respond. So when updating sectors, either the existing data has to be read first, or perhaps the bytes that don't need updating can be ignored with '0xff'. Something to explore.

There are other functions in this code block, some of them are more obvious than others. There are functions which utilise the MCU but are for flash ROMs with a different block size (for example). There are functions which don't interact with the MCU at all, and instead interface with the SPI subsystem. These functions often check the 'Boot BIOS Straps' which suggest that it's for writing boot firmware (as opposed to ME) attached to the SPI interface, but I'm slightly surprised it's in the same phlash interface.

And that's about it, as mentioned previously a port of 'flashrom' was developed in tandem to test information found here. Re-implementing these commands in it, allowed them to be tested. At the time of writing only small tests have been done, but the major functions reading, erase, & writing have been tested (writing/erase only tested on a single sector so far). The next step is to analyse the ROM image to better understand it's contents as the aim is still to update the BIOS/firmware. One curious point is that it looks like the firmware is not a BIOS, but an EFI firmware implementation (there are references to PIE, DXE, etc). This is curious because during the installation of the OS, the legacy version of GRUB had to be installed rather than the EFI version. This would be a relatively early implementation of EFI and there are reports of EFI implementations forcing the use of a CSM (compatability support module) to just boot using the MBR. This could make creating a new firmware image easier however, as it would mean just replacing the DXE section with Tianocore or just the BDS section for LinuxBoot. This is because it's the PEI (Pre-EFI Initialisation) which handles the 'hard part' of bringing up the system before handing off to the DXE stage. One problem to watch for however is that the firmware image is almost certainly 'verfied' on boot as there are references to regenerating checksums when executing phlash.
